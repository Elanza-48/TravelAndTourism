...
CREATE TABLE tourInfo(
    ...
    CONSTRAINT tInfo_hInfo_tp1 FOREIGN KEY (T_PLACE_1) 
        REFERENCES hotelInfo(T_PLACE)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT tInfo_hInfo_tp2 FOREIGN KEY (T_PLACE_2) 
        REFERENCES hotelInfo(T_PLACE)
        ON DELETE SET NULL ON UPDATE CASCADE,

    CONSTRAINT tInfo_hInfo_tp3 FOREIGN KEY (T_PLACE_3) 
        REFERENCES hotelInfo(T_PLACE)
        ON DELETE SET NULL ON UPDATE CASCADE
);
...
CREATE TABLE bookingInfo(
    ...
    CONSTRAINT bInfo_uAcc_umail FOREIGN KEY (U_EMAIL) 
        REFERENCES userAccount(U_EMAIL)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT binfo_tInfo_tid FOREIGN KEY (T_ID)
        REFERENCES tourInfo(T_ID)
        ON DELETE CASCADE ON UPDATE CASCADE
);